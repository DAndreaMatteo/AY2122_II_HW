...
//        slaveBuffer[2] = avg_LDR >> 8;
//        slaveBuffer[3] = avg_LDR & 0xFF;
//        slaveBuffer[4] = avg_TMP >> 8;
//        slaveBuffer[5] = avg_TMP & 0xFF;
        slaveBuffer[2] = light_lux;
        slaveBuffer[3] = temp_celsius;        
...
CY_ISR(Custom_Timer_ADC_ISR)
{
    ...
    if(counter_samples == num_avg_samples) ///< After num_avg_samples has been reached, the buffer can be updated
    {
        ...
        temp_celsius = (avg_TMP - 500)/10;
        
        // Conversion from mV to resistance (kOhm) and then conversion in lux
